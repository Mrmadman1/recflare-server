	...
	.get(
		...
		describeRoute({
			...
			description: [
				...
				'`creatorAccountId` narrows to one author’s items — what a player’s creations list',
				'sends (`creatorAccountId=<id>&includeCoachItems=False`); still published items only.',
			...
			parameters: [
				...
				},
				{
					...
					in: 'query',
					required: false,
					description: 'Only items this account made',
					schema: { type: 'integer' },
				},
				{
		...
		async (c) => {
			...
			const items = await searchCustomAvatarItems(c.env.DB, {
				...
				outfitTypes,
				includeCoachItems,
				creatorAccountId: int('creatorAccountId'),
