...
- A gift box's `AvatarItemType` (`econ`: the stored box in `GET /api/avatar/v2/gifts`, `buyItem`'s
  ...
  `www`'s staff boxes; `api`'s `gifts/generate`) must be NULL for anything that is not an avatar
  item — a consumable, a skin, a box, tokens, XP — and a NUMBER (the type, or 0) when the box holds
  one, whether a baked item by `AvatarItemDesc` or a custom item by `CustomAvatarItemId`. The
  client routes the box on that field before it reads what else the box names, so a 0 sends it
  looking for avatar item type 0 and it fails with "can't find avatar item". The 2025 store lists
